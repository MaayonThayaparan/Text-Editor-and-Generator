...
public class WPTree implements WordPath {
	...
	// Create NearbyWords object here.
	public WPTree () {
		this.root = null;
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		int count = 0; //used to terminate if meets threshold
		
		while (!queue.isEmpty() && !curr.getWord().equals(word2)) {
			curr = queue.removeFirst(); 
			List<String> currNeighbors = nw.distanceOne(curr.getWord(), true); 
			for (String n : currNeighbors) {
				count++; 
				if (count == THRESHOLD) { //shuts down search once threshold is met
					return new LinkedList<String>(); 
				}
				if (!visited.contains(n)) {
					WPTreeNode addedNode = curr.addChild(n);
					visited.add(n); 
					queue.addLast(addedNode); 
					if (n.equals(word2)) {
						return addedNode.buildPathToRoot();
					}
				}
			}
		}
