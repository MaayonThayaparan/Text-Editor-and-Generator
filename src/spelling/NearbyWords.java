...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		int count = 0; //used to cutoff search if it looks at # 
		...
		while (!queue.isEmpty() && retList.size() < numSuggestions) {
			...
			String currWord = ((LinkedList<String>) queue).removeFirst(); 
			List<String> neighbors = distanceOne(currWord, false);
			
			for (String n : neighbors) {
				
				count++;
				if (count == getThreshold()) { //shuts down search once threshold is met
					return retList; 
				}
				
				if (!visited.contains(n)) {
					visited.add(n); 
					((LinkedList<String>) queue).addLast(n); 
					if (dict.isWord(n)) {
						retList.add(n); 
					}
				}
   ...
   public static void main(String[] args) {
	   ...
	   word = "kangaroo";
	   List<String> suggestB = w.suggestions(word, 10);
	   System.out.println("Spelling Suggestions for \""+word+"\" are:");
	   System.out.println(suggestB);
   }
